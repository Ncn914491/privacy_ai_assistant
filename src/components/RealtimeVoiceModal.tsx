...
import { X, Mic, Square, AlertCircle, Wifi, WifiOff, Shield } from 'lucide-react';
import { cn } from '../utils/cn';
import { useRealtimeSTT } from '../hooks/useRealtimeSTT';
import { usePythonBackendLLM } from '../hooks/usePythonBackendLLM';
import { getPermissionStatusMessage, getPlatformPermissionInstructions } from '../utils/microphonePermissions';
...
export const RealtimeVoiceModal: React.FC<RealtimeVoiceModalProps> = ({
  ...
  const getConnectionStatus = () => {
    ...
    if (micPermission === 'unavailable') return { icon: Shield, text: 'Microphone unavailable', color: 'text-red-500' };
    if (micPermission === 'denied') return { icon: AlertCircle, text: 'Microphone access denied', color: 'text-red-500' };
    if (micPermission === 'checking') return { icon: AlertCircle, text: 'Checking microphone...', color: 'text-yellow-500' };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        ...
        <div className="p-6">
          ...
          {micPermission === 'unavailable' && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              ...
              <div className="text-red-700 text-sm mb-3">
                {getPermissionStatusMessage(micPermission, error || undefined)}
              </div>
              <div className="text-xs text-red-600 bg-red-100 p-2 rounded">
                {getPlatformPermissionInstructions()}
              </div>
            </div>
          )}
          ...
          {(micPermission === 'prompt' || micPermission === 'checking') && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center gap-2 text-yellow-600 mb-2">
                <AlertCircle className="w-4 h-4" />
                <span className="text-sm font-medium">
                  {micPermission === 'checking' ? 'Checking microphone...' : 'Microphone permission needed'}
                </span>
              </div>
              <div className="text-yellow-700 text-sm mb-3">
                {getPermissionStatusMessage(micPermission)}
              </div>
              <button
                onClick={handleRequestMicPermission}
              ...
              >
                {micPermission === 'checking' ? 'Checking...' : 'Request Permission'}
              </button>
            </div>
          )}
